...
async function loadBuildingsData() {
    try {
        // Try to load from the built version first, then fallback to source
        let response = await fetch('./buildings-data.json');
        if (!response.ok) {
            response = await fetch('./src/static/buildings-data.json');
        }
